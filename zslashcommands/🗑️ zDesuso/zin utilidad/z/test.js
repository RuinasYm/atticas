...
module.exports = {
    ...
    async execute (client, interaction, args) {
        ...
        interaction.channel.send({ embeds: [embed], components: [ops] }).then(async(m) => {

            const interaction = m.createMessageComponentCollector({ });
            interaction.on("collect", async(i) => {

                let rol = interaction.values[0]
                if (rol === "chat") { 
