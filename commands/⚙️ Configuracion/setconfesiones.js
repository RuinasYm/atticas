...
module.exports = {
  name: "setconfesiones",
  alias: ["setupconfesiones",],
  desc: "Sirve para responder automaticamente a un palabra",
  uso: "!setconfesiones <enable> <disable> <canal>",
  ...
  async execute(client, message, args, prefix) {
    ...
    if (!args.length) {
      return message.reply({
        ...
        .setColor("e60ad4")
        .setDescription(`<:fail:1083542064924479549> |  Uso correcto: \`${prefix}confesiones\` \`<enable>\` \`<disable>\` \`<canal>\`! | <:fail:1083542064924479549>`)
        ]
    });
    }
    ...
    if(!data) {
      ...
      let server = new Setups({
        confesionesEnabled: false,
        confesionesChannel: null,
    ...
    if (args[0].toLowerCase() === 'disable') {
        ...
        if (!data.confesionesEnabled) {
            return message.reply(`Las confesiones ya estan deshabilitadas <:coffe:1085588495172632638>.`)
        }
      // Actualizamos el valor de confesionesEnabled a false
      await Setups.findOneAndUpdate({guildID: message.guild.id}, {
        confesionesEnabled: false
      });
      // Enviamos un mensaje confirmando la acción
      return message.reply(`confesiones deshabilitadas <:coffe:1085588495172632638>.`);
    }
    ...
    switch (args[0]) {
      case "enable":{
          ...
          .setColor("e60ad4")
          .setDescription(`<:fail:1083542064924479549> |  Uso correcto: \`${prefix}setconfesiones\` \`<enable>\` \`<disable>\` \`<canal>\`! | <:fail:1083542064924479549>`)
...
await Setups.findOneAndUpdate({guildID: message.guild.id}, {
    confesionesEnabled: true,
    confesionesChannel: canal,
    guildID: message.guild.id,

}).then(() => {
  return message.reply({embeds: [new Discord.MessageEmbed()
      .setAuthor({name: 'confesiones habilitadas', iconURL: client.user.displayAvatarURL({dynamic: true})})
      .setThumbnail(client.user.displayAvatarURL({dynamic: true}))
